"""
Netlify Serverless Function for Pipedrive to Katana Order Sync
"""
import json
import os
import requests
...
DEFAULT_TAX_RATE_ID_EUR = 423653  # 23% VAT
DEFAULT_TAX_RATE_ID_GBP = 459884  # 20% VAT
CUSTOM_ITEM_VARIANT_ID = 38207669

def katana_request(endpoint, method='GET', data=None, params=None):
    """Make Katana API request"""
...
def find_katana_order_by_number(order_no):
    """Find existing Katana order by order number"""
...
def find_or_create_product(product_name, sku):
    """Find or create product in Katana"""
...
def find_or_create_variant_by_sku(sku, product_name, price, vat_rate):
    """Find variant by SKU, or create product/variant if not found"""
...
def get_tax_rate_id(vat_rate, currency='EUR'):
    """Get tax rate ID based on VAT rate and currency"""
...
def format_katana_date(won_time):
    """Convert Pipedrive won_time to Katana format"""
    if 'T' in won_time:
...
def build_additional_info(custom_items):
    """Build additional_info field for custom items"""
    if not custom_items:
        return ''
    ...
    try:
        ...
        if not deal_id:
            return {
                'statusCode': 400,
                'headers': {'Content-Type': 'application/json'},
                'body': json.dumps({'success': False, 'error': 'deal_id required'})
            }
        ...
        if not data.get('products'):
            ...
            if not deal_products:
                return {
                    'statusCode': 400,
                    'headers': {'Content-Type': 'application/json'},
                    'body': json.dumps({'success': False, 'error': 'No products in deal'})
                }
            ...
            for dp in deal_products:
                ...
                sku = (product_details.get(SKU_FIELD_KEY, '') or 
                       product_details.get('code', '') or '').strip()
                
                vat_rate = product_details.get('tax') or product_details.get('vat') or 23
                
                products.append({
                    'name': dp.get('name') or product_details.get('name', 'Unknown'),
                    'sku': sku,
                    'quantity': int(dp.get('quantity', 0)),
                    'price_per_unit': float(dp.get('item_price', 0)),
        ...
        if not data.get('won_time'):
            return {
                'statusCode': 400,
                'headers': {'Content-Type': 'application/json'},
                'body': json.dumps({'success': False, 'error': 'won_time required'})
            }
        ...
        if existing_order:
            return {
                ...
                'body': json.dumps({
                    ...
                    'order_no': existing_order.get('order_no')
                })
            }
        ...
        if not customer:
            return {
                'statusCode': 500,
                'headers': {'Content-Type': 'application/json'},
                'body': json.dumps({'success': False, 'error': 'Failed to create customer'})
            }
        ...
        for idx, product in enumerate(products, 1):
            ...
            if not sku:
                # No SKU - use CUSTOM-ITEM
                row = {
                    'variant_id': CUSTOM_ITEM_VARIANT_ID,
                    'quantity': quantity,
                    'price_per_unit': max(0, price),
                    'tax_rate_id': get_tax_rate_id(vat_rate, currency),
                ...
                # Has SKU - find or create variant
                ...
                if variant:
                    ...
                    # Use CUSTOM-ITEM if variant creation failed
                    row = {
                        'variant_id': CUSTOM_ITEM_VARIANT_ID,
                        'quantity': quantity,
                        'price_per_unit': max(0, price),
        ...
        if not order_rows:
            return {
                'statusCode': 400,
                'headers': {'Content-Type': 'application/json'},
                'body': json.dumps({'success': False, 'error': 'No products to add'})
            }
        ...
        order_data = {
            ...
            'customer_ref': data.get('deal_title', ''),
            'order_created_date': format_katana_date(won_time),  # Matches won_time
            'delivery_date': calculate_delivery_date(won_time),
            'currency': data.get('currency', 'EUR'),
            'location_id': DEFAULT_LOCATION_ID,
            'source': 'pipedrive',
